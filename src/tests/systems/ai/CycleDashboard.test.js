/**
 * CycleDashboard.test.js - 轮回仪表盘测试
 * V375 Iteration 9/9 FINAL Round 10 - 测试覆盖率目标: 99%+
 */

import { describe, it, expect, beforeEach } from 'vitest';
import { CycleDashboard } from '../../../systems/ai/CycleDashboard.js';

describe('CycleDashboard', () => {
    let system;
    beforeEach(() => { system = new CycleDashboard(); });
    ...
    describe('setMetrics', () => {
        it('should set', () => {
            const { dashboard } = system.createDashboard({});
            const result = system.setMetrics(dashboard.dashboardId, { cycleCount: 5 });
    ...
    describe('calculateCycleProgress', () => {
        it('should calculate', () => {
            const { dashboard } = system.createDashboard({});
            system.setMetrics(dashboard.dashboardId, { cycleCount: 5 });
            expect(system.calculateCycleProgress(dashboard.dashboardId)).toBe(0.5);
        });

        it('should cap at 1', () => {
            const { dashboard } = system.createDashboard({});
            system.setMetrics(dashboard.dashboardId, { cycleCount: 100 });
            expect(system.calculateCycleProgress(dashboard.dashboardId)).toBe(1);
        });

        it('should return null for missing', () => {
            expect(system.calculateCycleProgress('ghost')).toBeNull();
        });
    });
    ...
    describe('calculateKarmaScore', () => {
        it('should calculate', () => {
            const { dashboard } = system.createDashboard({});
            system.setMetrics(dashboard.dashboardId, { karmicBalance: 100 });
            expect(system.calculateKarmaScore(dashboard.dashboardId)).toBe(10);
        });

        it('should return 0 for missing', () => {
            expect(system.calculateKarmaScore('ghost')).toBe(0);
        });
    });
    ...
    describe('Persistence', () => {
        ...
        it('should deserialize', () => {
            system.createDashboard({});
            const json = system.toJSON();
            const newSys = new CycleDashboard();
