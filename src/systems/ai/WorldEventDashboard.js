/**
 * WorldEventDashboard.js - 异变仪表盘
 * V393 Iteration 9/9 FINAL Round 12
 */
export class WorldEventDashboard {
    constructor(config = {}) {
    ...
    createDashboard(data) {
        const id = data.id || `wed_${Date.now()}_${Math.random().toString(36).slice(2, 7)}`;
        const dashboard = { dashboardId: id, name: data.name || 'World Event Dashboard', region: data.region, createdAt: Date.now(), lastRefresh: Date.now() };
        this.dashboards.set(id, dashboard);
        this.metrics.set(id, { activeEvents: 0, ongoingCatastrophes: 0, ongoingTribulations: 0, aliveBosses: 0, activeWarnings: 0, averageSeverity: 0, totalFavor: 0 });
    ...
    calculateThreatLevel(dashboardId) {
        const metrics = this.getMetrics(dashboardId);
        if (!metrics) return null;
        const score = (metrics.activeEvents || 0) * 1 + (metrics.ongoingCatastrophes || 0) * 3 + (metrics.ongoingTribulations || 0) * 2 + (metrics.aliveBosses || 0) * 4;
        if (score > 50) return 'catastrophic';
    ...
    calculateStabilityIndex(dashboardId) {
        const metrics = this.getMetrics(dashboardId);
        if (!metrics) return 0;
        const threats = (metrics.activeEvents || 0) + (metrics.ongoingCatastrophes || 0) + (metrics.aliveBosses || 0);
        return Math.max(0, 100 - threats * 10);
    }
