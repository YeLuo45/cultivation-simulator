// ===== V73 TDD Tests: MCP Agent Bridge =====

function runV73Tests() {
    const results = [];
    ...
    function assertContains(str, substr, message) {
        if (!str || !str.includes ? !str.includes(substr) : !String(str).includes(substr)) {
    ...
    test('T1: MCP_TOOLS 定义了8个工具', () => {
        ...
        assert(MCP_TOOLS['npc.query'], 'npc.query 存在');
        assert(MCP_TOOLS['serendipity.trigger'], 'serendipity.trigger 存在');
        assert(MCP_TOOLS['cultivation.advance'], 'cultivation.advance 存在');
        assert(MCP_TOOLS['item.exchange'], 'item.exchange 存在');
        assert(MCP_TOOLS['gameState.query'], 'gameState.query 存在');
        assert(MCP_TOOLS['battle.start'], 'battle.start 存在');
        assert(MCP_TOOLS['mcp.providers'], 'mcp.providers 存在');
        assert(MCP_TOOLS['mcp.switch_provider'], 'mcp.switch_provider 存在');
    });
    ...
    test('T3: cultivationMCPServer 是 CultivationMCPServer 实例', () => {
        assert(cultivationMCPServer instanceof CultivationMCPServer, '应为 CultivationMCPServer 实例');
    });
    ...
    test('T14: gameState.query(realm) 返回realm/stage/level', () => {
        ...
        assertEqual(result.realm, gameState.realm, 'realm值');
        assertEqual(result.stage, gameState.stage, 'stage值');
        assertEqual(result.level, gameState.level, 'level值');
    });
    ...
    test('T22: 无NPC系统时mcpNpcQuery返回错误', () => {
        // 临时移除NPC系统
        const original = gameState.npcCollab;
        gameState.npcCollab = null;
        const result = cultivationMCPServer.mcpNpcQuery('test_npc', 'info');
        assert(result.error, '应返回错误');
        gameState.npcCollab = original;
    });
...
const v73Results = runV73Tests();
console.log(`V73 MCP Agent Bridge Tests: ${v73Results.passed}/${v73Results.passed + v73Results.failed} passed`);
if (v73Results.failed > 0) {
    v73Results.results.filter(r => r.status === 'FAIL').forEach(r => {
