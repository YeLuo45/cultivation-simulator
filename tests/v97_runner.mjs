// V97 TDD Test Runner - ESM version
import { readFileSync } from 'fs';
import { createRequire } from 'module';
const require = createRequire(import.meta.url);

// Read game.js
const gameContent = readFileSync('game.js', 'utf8');

// Create a proper sandbox context
const contextDict = {
    console: console,
...
// Use vm with a custom runInContext that handles script properly
import vm from 'vm';
const script = new vm.Script(gameContent, { filename: 'game.js' });
const context = vm.createContext(contextDict);

try {
    script.runInContext(context);
} catch (e) {
    // Some TDZ issues at end of file - try to continue
    if (e.message.includes('Cannot access') || e.message.includes('before initialization')) {
        console.log('Note: Some tests at end of file failed due to TDZ (expected)');
    } else {
        console.log('Error loading game.js:', e.message);
    }
}

if (typeof context.CultivationMCPServer !== 'function') {
    console.log('ERROR: CultivationMCPServer not found in context');
    console.log('Available keys:', Object.keys(context).filter(k => typeof context[k] === 'function'));
    process.exit(1);
}
...
test('V97: MCP_TOOLS_V97 constant defined', () => typeof context.MCP_TOOLS_V97 === 'object' && Object.keys(context.MCP_TOOLS_V97).length === 6);
test('V97: All 6 tools registered', () => v97Tools.every(t => server.toolRegistry.has(t)));
test('V97: market.skills.list schema valid', () => context.MCP_TOOLS_V97['market.skills.list']?.inputSchema);
test('V97: market.skills.buy schema valid', () => context.MCP_TOOLS_V97['market.skills.buy']?.inputSchema?.required?.includes('skillId'));
test('V97: market.skills.sell schema valid', () => context.MCP_TOOLS_V97['market.skills.sell']?.inputSchema?.required?.includes('skillId'));
test('V97: skill.learn schema valid', () => context.MCP_TOOLS_V97['skill.learn']?.inputSchema?.required?.includes('skillId'));
test('V97: skill.invoke schema valid', () => context.MCP_TOOLS_V97['skill.invoke']?.inputSchema?.required?.includes('skillId'));
test('V97: sect.war.preview schema valid', () => context.MCP_TOOLS_V97['sect.war.preview']?.inputSchema?.required?.includes('sectId'));
...
if (skill2) {
    context.window.gameState.spiritStones = 1000;
