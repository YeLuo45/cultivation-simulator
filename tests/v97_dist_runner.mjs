// V97 TDD Test Runner - Using pre-built dist/game.js
import { readFileSync } from 'fs';
import { createRequire } from 'module';
const require = createRequire(import.meta.url);

const gameContent = readFileSync('dist/game.js', 'utf8');
...
import vm from 'vm';
const script = new vm.Script(gameContent, { filename: 'dist/game.js' });
...
try {
    ...
    if (e.message.includes('Cannot access') || e.message.includes('before initialization')) {
        console.log('Note: Some TDZ issues at end of file (expected for this test runner)');
    } else {
        console.log('Error loading dist/game.js:', e.message);
    }
}

if (typeof context.CultivationMCPServer !== 'function') {
    console.log('ERROR: CultivationMCPServer not found');
    console.log('Available keys starting with C:', Object.keys(context).filter(k => k.startsWith('C')));
    process.exit(1);
}
