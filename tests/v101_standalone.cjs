// V101 Standalone Test Runner - Execute TDD tests for 仙盟系统 仙盟创建+领地争夺
// This loads the game code and runs tests

const path = require('path');
const fs = require('fs');

console.log('=== V101 TDD Test Runner ===\n');
...
// Load game code to get MCP_TOOLS_V101 and CultivationMCPServer
try {
    // Execute game code to get access to globals
    eval(gameCode);
    console.log('✓ Game code loaded\n');
} catch (e) {
    console.error('✗ Failed to load game code:', e.message);
    process.exit(1);
}
...
results.push(test('V101: Multiple alliances can coexist', () => {
    server.mcpAllianceCreate({ name: '仙盟一' });
    server.mcpAllianceCreate({ name: '仙盟二' });
    server.mcpAllianceCreate({ name: '仙盟三' });
